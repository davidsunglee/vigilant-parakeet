...
- **Durable task runner (Trigger.dev):** Runs the `generate-story` task server-side with Claude Sonnet (text) + GPT-4 Vision (images), persisting checkpoints and images so retries resume, not restart.
