import type { ILlmProvider, JsonSchema } from '../providers/types';
import type {
  IAnimalEntity,
  ITraitChecklist,
  IAnimalVisualDescription,
  IStoryVisualAnchor,
} from '../types/story.types';

/**
 * Relocated from apex/src/services/LlmService.ts — keep prompt strings and JSON
 * schemas in sync. The browser `fetch('/api/llm/generate')` proxy call is
 * replaced with a direct adapter call; `model`/`userId` are supplied at
 * construction so the pipeline does not have to thread provider config.
 */
export class LlmClient {
  ...
  private async callLlm(prompt: string, responseSchema: JsonSchema, systemPrompt?: string): Promise<any> {
  ...
  async getAnimalProfile(animalName: string): Promise<Omit<IAnimalEntity, 'id' | 'commonName'>> {
    const data = await this.callLlm(`Provide biological stats and habitat for the animal: ${animalName}`, {
      type: 'object',
      properties: {
...
Do not contradict or alter the animals' species, colors, markings, or art style described above.`;
  ...
  async getAnimalVisualDescriptions(
    animalA: IAnimalEntity,
    animalB: IAnimalEntity,
