...
describe('AnthropicLlmAdapter', () => {
  ...
  describe('generate()', () => {
    ...
    it('passes metadata.user_id when userId is provided', async () => {
      const { adapter, create } = buildAdapter([
        { type: 'tool_use', id: 't1', name: 'structured_output', input: { name: 'A' } },
      ]);
      await adapter.generate({ ...objectSchema, userId: 'user-123' });
      const callArgs = create.mock.calls[0][0];
      expect(callArgs.metadata).toEqual({ user_id: 'user-123' });
    });
  });
});
